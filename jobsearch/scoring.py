...
def _цитата_настоящая(цитата: str, cv: str) -> bool:
    ...
    Сверяем по словам, а не по буквам: модель переставляет пробелы, меняет
    регистр и роняет знаки препинания, и придираться к этому значило бы
    отбрасывать честные цитаты. А вот выдуманное так не совпадёт: «опыт в
    области электротехники» не найдётся в резюме, где её нет.
    """
    слова = re.findall(r"\w+", (цитата or "").lower())
    if len(слова) < 2:
        return False
    в_резюме = " ".join(re.findall(r"\w+", (cv or "").lower()))
    return " ".join(слова) in в_резюме
...
ПЕРЕВОД_ПРОМПТ = """Переведи название вакансии на английский. Ответь ТОЛЬКО переводом,
одной строкой, без пояснений и кавычек. Если название уже английское — повтори его как есть.

Название: {title}"""


def translate_title(job: dict, cfg: dict, ask: dict, по_английски: bool = False) -> str:
    """Название вакансии по-английски — тем объявлениям, что написаны иначе.

    Справочник профессий закрывает только EURES: у него есть код занятия. А
    шведская «Hemfrid Skövde söker städspecialist» приходит с JobTech, кода у
    неё нет, и местная модель, не прочитав ни слова, поставила ей девяносто
    процентов. Так же и французские «Modéliste», которых у конструктора белья
    набралось двенадцать штук на 50–55%: её собственная должность, не узнанная.

    Переводим только название — три-пять слов. Перевести куда проще, чем судить,
    и на это способна даже малая модель; целое объявление ей не по силам ни там,
    ни там, но вердикт решается по названию.

    Язык определяет вызывающий — по описанию, а не по названию. В названии слов
    пять, служебных среди них не бывает, и любая проверка на нём отвечает
    «сойдёт за английское»: первая редакция не перевела ни одного названия,
    включая «Modéliste Flou PAP mode Femme Luxe».

    Не вышло — возвращаем пустое: пусть лучше модель судит как раньше, чем
    прогон встанет из-за перевода.
    """
    название = (job.get("title") or "").strip()
    if not название or job.get("occupation") or по_английски:
        return ""
    try:
        ответ = llm.ask(ПЕРЕВОД_ПРОМПТ.format(title=название[:200]), timeout=120, **ask)
    except (llm.ClaudeError, llm.AuthError):
        return ""
    перевод = str(ответ or "").strip().splitlines()[0].strip(" \"'.").strip()
    # Модель иногда отвечает объяснением вместо перевода. Название вакансии
    # длиннее полутора сотен знаков не бывает — значит это не перевод.
    return перевод[:150] if 0 < len(перевод) <= 150 else ""
...
def _occupation_block(cfg: dict, job: dict) -> str:
    ...
    # Перевод названия — то же самое другими средствами: тем объявлениям, у
    # которых кода профессии нет, а написаны они не по-английски.
    if not занятие and job.get("title_en"):
        занятие = job["title_en"].strip()
        своя = (cfg["profile"].get("roles") or "").split(",")[0].strip()
        # Приблизительный — и сказано об этом прямо. Перевод делает та же малая
        # модель, и на пробе она перевела «Szwaczka» (швея) как «Cleaner», а
        # «Modéliste Flou PAP» — как «Visual Merchandiser»: два верных перевода
        # из четырёх. Выдавать такое за истину значило бы поменять одну ошибку
        # на другую; пусть модель видит и исходное название, и приблизительный
        # перевод, и знает, какой чему цена.
        строки = [f"Объявление написано не по-английски. Машинный перевод "
                  f"названия (может быть неточен): «{занятие}». "
                  f"Исходное название выше — если оно тебе понятнее, верь ему."]
        if своя:
...
def deep_analyze(job: dict, cfg: dict, cv: str, log, research: bool = True) -> None:
    ...
    #
    # Название по-английски — тем объявлениям, что написаны иначе и кода
    # профессии не имеют. Без него модель судила по строке, которой не понимала.
    английское = _по_английски(f"{job.get('title', '')} {description[:1500]}")
    if not job.get("occupation") and not job.get("title_en"):
        job["title_en"] = translate_title(job, cfg, ask, по_английски=английское)
    ...
    if isinstance(result.get("match"), (int, float)):
        ...
        # непрочитано.
        # Перевод названия за понятое НЕ считается, хотя вердикт по нему и
        # ставится. Перевод делает та же малая модель, и на пробе «Szwaczka»
        # (швея) стала «Cleaner»: это не второй внимательный проход, а та же
        # догадка дважды. Судить по нему лучше, чем по непонятной строке, а
        # ручаться за него — нет.
