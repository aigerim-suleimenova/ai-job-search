...
def eures(cfg: dict, log) -> list:
    ...
    for term in слова[:4]:
        for uri in esco.occupations(term):
            if uri not in коды:
                коды.append(uri)
    # И профессии, выведенные из одних навыков. Это про тех, у кого роль
    # обманывает: охранник, выучивший вёрстку, напишет в резюме «охранник», и
    # вакансий начинающего верстальщика не увидит никогда. Справочник по трём
    # его навыкам называет web developer и webmaster, ничего не спрашивая.
    #
    # Отдельным запросом, а не вперемешку с ролями: у конструктора белья
    # справочник знает один навык из восьми, и тот ведёт к обувным профессиям —
    # подмешав их к ролям, мы испортили бы то, что уже работает.
    навыки = [н.strip() for н in (cfg["profile"].get("skills") or "").split(",") if н.strip()]
    по_навыкам = [u for u in esco.occupations_by_skills(навыки[:8]) if u not in коды]
