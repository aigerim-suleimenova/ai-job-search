...
TR = {
    ...
    "models_websearch_warning": {"ru": "Выбранная модель не умеет искать в интернете. Настраивать компании вручную из-за этого не придётся: вакансии по-прежнему собираются с {n} источников, включая доски самих работодателей, а список компаний пополняется сам — по ссылкам внутри уже найденных вакансий. Не будет двух вещей: модель не пойдёт искать по всему интернету, где нашлись бы компании и вне этих источников, и не соберёт зарплатные вилки и сведения о компаниях.",
                                 "en": "The selected model cannot search the web. That does not leave you setting up companies by hand: jobs still come from {n} sources, employers' own job boards among them, and the list of companies still grows on its own, from the links inside the jobs already found. Two things are lost: the model will not go looking across the open web, where companies outside those sources would turn up, and it will not gather salary ranges and facts about companies.",
                                 "it": "Il modello scelto non può cercare sul web. Non per questo dovrà configurare le aziende a mano: le offerte continuano ad arrivare da {n} fonti, fra cui le bacheche delle aziende stesse, e l'elenco delle aziende cresce da sé, dai link dentro le offerte già trovate. Si perdono due cose: il modello non andrà a cercare su tutto il web, dove emergerebbero aziende al di fuori di queste fonti, e non raccoglierà fasce salariali e informazioni sulle aziende.",
                                 "de": "Das gewählte Modell kann nicht im Web suchen. Unternehmen von Hand einzutragen bleibt Ihnen dennoch erspart: Stellen kommen weiterhin aus {n} Quellen, darunter die Stellenbörsen der Arbeitgeber selbst, und die Liste der Unternehmen wächst von selbst — aus den Links in den bereits gefundenen Stellen. Zweierlei entfällt: Das Modell sucht nicht im gesamten Web, wo Unternehmen außerhalb dieser Quellen auftauchen würden, und es sammelt keine Gehaltsspannen und Angaben zu Unternehmen."},
