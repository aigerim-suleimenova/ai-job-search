...
TR = {
    ...
    "models_websearch_warning": {"ru": "Выбранная модель не умеет искать в интернете. Настраивать компании вручную из-за этого не придётся: вакансии собираются с {n} источников, среди них доски самих работодателей — там объявление идёт прямо от компании, — а список компаний пополняется сам, по ссылкам внутри уже найденных вакансий. Не будет одного: разведки по всему интернету. Из-за неё же не соберутся зарплатные вилки и сведения о компаниях — их модель берёт из выдачи, а брать будет неоткуда.",
                                 "en": "The selected model cannot search the web. That does not leave you setting up companies by hand: jobs come from {n} sources, among them employers' own job boards, where the posting comes straight from the company — and the list of companies still grows on its own, from the links inside the jobs already found. One thing is lost: scouting the open web. Because of that, salary ranges and facts about companies will not be gathered either — the model takes those from search results, and there will be none.",
                                 "it": "Il modello scelto non può cercare sul web. Non per questo dovrà configurare le aziende a mano: le offerte arrivano da {n} fonti, fra cui le bacheche delle aziende stesse, dove l'annuncio viene direttamente da loro — e l'elenco delle aziende cresce da sé, dai link dentro le offerte già trovate. Si perde una cosa: la ricognizione su tutto il web. Per lo stesso motivo non si raccoglieranno fasce salariali e informazioni sulle aziende: il modello le prende dai risultati di ricerca, e non ce ne saranno.",
                                 "de": "Das gewählte Modell kann nicht im Web suchen. Unternehmen von Hand einzutragen bleibt Ihnen dennoch erspart: Stellen kommen aus {n} Quellen, darunter die Stellenbörsen der Arbeitgeber selbst, wo die Anzeige direkt vom Unternehmen kommt — und die Liste der Unternehmen wächst weiterhin von selbst, aus den Links in den bereits gefundenen Stellen. Eines entfällt: die Erkundung des offenen Webs. Deshalb kommen auch keine Gehaltsspannen und Angaben zu Unternehmen zusammen — die nimmt das Modell aus den Suchergebnissen, und die wird es nicht geben."},
