...
- **macOS** — открыть `AI Job Search.dmg`, перетащить приложение на ярлык
  Applications, запустить из Launchpad. Сборка подписана ad-hoc, поэтому при
  первом запуске система откажется её открыть и предложит переместить в
  Корзину. Нажать «Готово», затем: Системные настройки → «Конфиденциальность
  и безопасность» → пролистать вниз до сообщения про «AI Job Search» →
  «Всё равно открыть». В старых статьях советуют правый клик → «Открыть»:
  начиная с macOS 15 это больше не работает. Из терминала то же самое —
  `xattr -dr com.apple.quarantine "/Applications/AI Job Search.app"`.
- **Windows** — распаковать архив, запустить `AI Job Search.exe`.
